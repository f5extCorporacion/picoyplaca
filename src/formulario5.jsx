...
function Formulario5() {
  ...
  const getPicoYPlacaDaysInMonth = (year, monthIndex, digit) => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      if (isPicoPlacaDay || (isHolidayDay && isPicoPlacaDay)) {
        picoDays.push({
          day: i,
  ...
  useEffect(() => {
    ...
    const groupedCart = selectedMonthsDetails.reduce((acc, item) => {
      const key = `${item.type}-${item.resolution}-${item.semestre}`;
      if (!acc[key]) {
        acc[key] = {
          type: item.type,
  ...
  const getMonthState = (monthIndex, resolution) => {
    ...
    return { isDisabled, isGreyedOut, picoPassedInCurrentMonth };
  };
  ...
  const getDayState = (monthIndex, day, resolution) => {
    ...
    const isDisabled = !isSelectable || isPastDay || isPastHourToday;
  ...
  const handleMonthCheckboxChange = (monthIndex, resolution, isChecked = null) => {
    const { isDisabled } = getMonthState(monthIndex, resolution);
  ...
  const handleDayButtonClick = (monthIndex, day, resolution) => {
    ...
    const { isPicoPlaca } = getDayState(monthIndex, day, resolution);
    
    if (!isPicoPlaca || isHoliday(date)) return;
 ...
 const validateForm = (values) => {
  ...
  // Validación de DV (independiente del tipo de documento)
  if (!values.dv) {
    errors.dv = 'El dígito de verificación es requerido.';
  } else if (isNaN(values.dv)) {
  ...
  // Validación de selección de días/meses
  ...
  // Validación de clase de vehículo
  ...
  setPrevCart(values);
  console.log(JSON.stringify(prevcart));
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm, setErrors }) => {
    try {
      ...
      // Validar que haya selección de días/meses
      if (lastDigit !== null && cart.length === 0) {
                          ...
                          <div className="accordion-body">
                            <div className="form-check mb-3">
                              <input
                                ...
                                checked={monthsForResolution.some(month => {
                                  const monthState = getMonthState(month.index, resolution);
                                  if (monthState.isDisabled) return false;
                                ...
                                monthsForResolution.every(month => {
                                  const monthState = getMonthState(month.index, resolution);
                                  if (monthState.isDisabled) return true;
                                  const currentMonthData = selectedMonths[month.index] || { isSelected: false };
                                  return currentMonthData.isSelected;
                                })}
                                ...
                                title="Seleccionar todos los meses disponibles de esta resolución"
                              />
                              <span className="form-check-label">Toda la resolución</span>
                            </div>
                            ...
                            <div className="semester-group">
                              {monthsForResolution.map((month) => {
                                const { isDisabled, isGreyedOut, picoPassedInCurrentMonth } = getMonthState(month.index, resolution);
                                  ...
                                  >
                                    <div className="mes form-check mb-2">
                                      <input
                                        ...
                                        disabled={monthCheckboxDisabled || lastDigit === null || !isActiva}
                                        title={isCurrentMonth && picoPassedInCurrentMonth ? "No se puede seleccionar el mes completo porque ya pasó al menos un día de pico y placa" : ""}
                                      />
                                          ...
                                          month.days.map(({day, isHoliday, isPicoPlaca}) => {
                                            ...
                                            return (
                                              <button
                                                ...
                                                onClick={() => isPicoPlaca && handleDayButtonClick(month.index, day, resolution)}
                                                disabled={isDisabled}
                                                title={
                                ...
                                <div className="text-end">
                                  ...
                                  {month.discount > 0 && (
                                    <small className="text-success">
                                      Descuento: {(month.discount * 100).toFixed(0)}%
                                    </small>
                                  )}
                                </div>
                              </div>
          ...
          <div className="text-center mt-4 d-flex justify-content-center gap-3">
            <button
              ...
              onClick={() => MySwal.fire({
                ...
                html: (
                  <div>
                    <PdfPreview  cart={cart} finalTotal={finalTotal} prevcart={prevcart}/>
